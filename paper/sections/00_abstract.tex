We present OMU-MAE (Occupancy + Multi-modal + Unified Masked
...
\emph{same} frozen-VFM target). We additionally report a nuScenes
cross-sensor transfer study under a frozen probe: the cross-modal target
is what transfers (both DINOv2-target variants lead the LiDAR-only,
contrastive and random baselines), while the in-domain benefit of masking
does not, a nuance we analyze honestly. The full
