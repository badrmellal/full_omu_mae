...
range-aware masking and reconstructs binary voxel occupancy. OMU-MAE keeps
its range-aware schedule but \emph{adds} a second target at masked
positions---a $768$-d vector matching the frozen DINOv2 patch feature
projected from the paired image---and trains both losses jointly.
...
NeRF-style volumetric rendering. OMU-MAE differs on three axes: the image
encoder is \emph{frozen}, the cross-modal target is the frozen DINOv2
\emph{feature} volume (not raw pixels), and there is no interaction module
or back-projection---only direct concatenation into the voxel input. The
trade-off is intentional: less expressive, but it trains in a few hours on
a single GPU.
