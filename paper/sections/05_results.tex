...
Per-class IoU at $100\%$ labels concentrates the OMU-MAE gains over random
init on the dominant geometric and semantic classes (building, sidewalk,
road, car, terrain, vegetation, fence, pole). Small or rare classes
(motorcyclist, bicyclist, person, bicycle, motorcycle) remain near-zero IoU
under \emph{all} conditions: at $0.4$\,m a small object spans only $1$--$2$
voxels, so a single linear classifier cannot recover its fine-grained
semantics regardless of the pretraining recipe.
...
classes sorted by OMU-MAE IoU. Right: per-class $\Delta$ IoU (pp) of OMU-MAE
over random; gains concentrate on the geometric, semantic classes.}
\label{fig:perclass}
\end{figure*}
\fi
